...
describe("BandContract ", function () {
  it("Deploys Proxy Band Contract and Creates Songs(NFT Collections)", async function () {
...
//  let  add = await alivecore.bandBeacon();
  ...
  let songDep3 = await Band.createSongDutch(
    ...
    _contractURI,
    _royaltyFeesInBips,
    artist.address, [100, ethers.utils.parseEther("3"), ethers.utils.parseEther("0.0003"), Date.parse(time)/1000 - 10, pricedecreasePerHour],  {gasLimit: 10000000}
  );
  ...
  assert.isTrue(!songId3< songId2, "new song id should be greater")
  ...
  console.log('balance before of royalty receiver',  ethers.utils.formatEther(await royaltyReceiver.getBalance()));
  console.log('balance before of minter', ethers.utils.formatEther(await manager.getBalance()));
  ...
  console.log('id', id);
  console.log('balance after',  ethers.utils.formatEther(await royaltyReceiver.getBalance()));
  console.log('price',    ethers.utils.formatEther(await dutch.getPrice(10)));
